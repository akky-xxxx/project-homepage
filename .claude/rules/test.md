...
- カバレッジ75%を目指す
- テスト失敗の原因を「自分の変更が原因」「フレーク」等と結論づける前に、設定ファイル・環境変数の実値・テストヘルパーが前提にしている定数を実際に読んで根拠を実測する。再実行して緑になったことは切り分けの根拠にならない
...
- レビュー指摘等を受けてテストを追加・修正した場合、修正前の実装を一時的に再現し、当該テストが実際に落ちることを確認してから元に戻す
- 再現は指摘当時の状態を正確に再現する。部分的に戻した状態のまま確認しない
- 再現を始める前に、対象ファイルに未コミットの差分が無いことを `git status --short <file>` で確認する。差分がある場合はこの手順(一時変更 + `git restore` による復元)を使わず、別の worktree で再現するなど、開始前の変更を破棄しない手段を取る
- 差分が無いことを確認できた場合に限り、復元には `git restore` を使う。`cp` によるバックアップからの復元は使わない(この環境では `cp` が `cp -i` にエイリアスされており、非対話の Bash では上書きせず `not overwritten` を出して黙って終了するため、復元したつもりで変更が残る事故につながる)
- 復元後は `git status --short` で差分が残っていないことを確認する
- 確認した内容(実行したコマンドと結果)を result の `## 確認済みのこと` に書く
- 非同期に反映される UI の検証では、直前の操作の結果が残っているだけで即座に成立し得る条件(同じ文言のバナー、遅延して空になる入力欄など)を成功条件に使わない
