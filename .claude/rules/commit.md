...
- cherry-pick や drop 等、コミット単位の操作をしやすい粒度
  ...
  - 実施例: 対象コミットが依存関係(`package.json`/`bun.lock`)を変更していない場合に限り、`git worktree` で別ディレクトリに切り出し(detached HEAD)、`node_modules` をシンボリックリンクして再インストールを避けつつ品質ゲート相当のコマンドを実行する。依存関係を変更するコミットでは、シンボリックリンクした `node_modules` が実際の依存内容とずれるため、通常通り `bun install` してから実行する
