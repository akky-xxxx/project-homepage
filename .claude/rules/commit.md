...
- cherry-pick や drop 等、コミット単位の操作をしやすい粒度
  - 実装における1機能以下
  - ここでいう「操作をしやすい」は、コミットが意図の単位でまとまっていて cherry-pick / drop した結果が変更内容として意味をなすことを指す。各コミット単体で品質ゲート相当のチェック(`bun check-code` 等)が通ることまでは求めない。品質ゲートは GitHub Actions が pull request と `develop`/`main` への push で実行するため、lint / format / 型チェック等の整合性は PR として提出する最終状態で通っていればよい
  - 分割の判断軸は「その変更が単独で説明できる1つの意図を持つか」。直前のコミットに対する lint/format 対応であっても、意図が別であれば分けてよい。単体で品質ゲートを通らないことを理由にコミットをまとめて粒度を大きくしない
