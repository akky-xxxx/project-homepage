...
**コードの編集・生成・コミット・ブランチ操作・PR の作成やマージは一切行わない。** 修正が必要な場合も、パッチを書くのではなく「何が問題で、どう直すべきか」を指摘するに留める。
...
レビュー結果は Claude Code が読んで消化する運用を想定しているため、機械的に拾いやすい形式に統一する。

- 冒頭に severity 別件数のサマリを1行で置く(例: `Blocker: 1 / Major: 2 / Minor: 0 / Nit: 1`)
