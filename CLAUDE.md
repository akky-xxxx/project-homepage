...
- retrospective 実行後、同じブランチへ新しいスコープが追加された場合: push 前に retrospective を再実行する。再実行は追加分のスコープに絞ってよく、既に起票済み・判断済みの項目を再掲しない
- retrospective の実行がユーザー操作待ちで中断した場合、またはユーザーの直接指示で push / PR 作成 / merge が先行した場合: 実行を省略せず事後に行う。呼び出し時の経緯メモに「push 等が先行した」ことを明記し、本来 push 前に行うはずだった「申し送りの別 issue 候補の未起票確認」もあわせて行う
